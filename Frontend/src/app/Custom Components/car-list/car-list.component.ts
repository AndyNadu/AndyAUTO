...
export class CarListComponent {
  ...
  constructor(private _componentInteractionService: ComponentInteractionService,
    ...
    this.imageSource = this._sanitizer.bypassSecurityTrustUrl(`data:image/jpg;base64, ${this.imgs}`);

    _http.get<Car[]>('http://localhost:5113/car/get')
      .subscribe(
        (res: Car[]) => {
          this.carsList = this.prepareMiniCarPost(res);
        },
    ...
    this.carsList.forEach(car => {
      car.firstImage = this.getBase64Image(car.imagesAsBase64Strings);
    });
  }
  ...
  prepareMiniCarPost(cars: Car[]): MiniViewCar[] {
    ...
    cars.forEach(car => {
      const _car: MiniViewCar = {
        ...
        mileage: this.formatNumber(car.mileage),
        description: car.description,
        fuel: car.fuel,
        cubicCapacity: this.formatNumber(car.cubicCapacity),
        power: this.formatNumber(car.power),
        ...
        postTime: this.formatDate(car.postTime),
        imagesAsBase64Strings: car.imagesAsBase64Strings,
        firstImage: car.firstImage
      };
